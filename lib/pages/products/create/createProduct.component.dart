...
class CreateProductComponent extends ChangeNotifier {
  ...
  Future<void> fetchBrandsByCategory(int categoryId) async {
    ...
    try {
      final url = Uri.parse('${EnvironmentDev.baseUrl}${EnvironmentDev.brandsByCategory(categoryId)}');
      final response = await http.get(url, headers: _getHeaders());
  ...
  Future<String> uploadImage(File imageFile, int productId, int imageOrder, bool isPrimary) async {
    try {
      final token = _getToken();
      final url = Uri.parse('${EnvironmentDev.baseUrl}${EnvironmentDev.productsUploadImage(productId)}');
  ...
  Future<void> createProduct(ProductModel product, {List<XFile>? images}) async {
    _isLoading = true;
    _error = null;
    notifyListeners();
    ...
    try {
      ...
      if (images != null && images.isNotEmpty) {
        print('Validando ${images.length} imágenes antes de crear el producto...');
        ...
        for (int i = 0; i < images.length; i++) {
          ...
          if (!await imageFile.exists()) {
            throw Exception('La imagen ${i + 1} no existe o no se puede acceder');
          }
          
          // Validar tamaño de archivo (máximo 10MB)
          final fileSize = await imageFile.length();
          if (fileSize > 10 * 1024 * 1024) {
            throw Exception('La imagen ${i + 1} es demasiado grande (máximo 10MB)');
          }
        }
      }
      
      // 2. Crear el producto sin imágenes
      final url = Uri.parse('${EnvironmentDev.baseUrl}${EnvironmentDev.productsCreate}');
      ...
      if (createResponse.statusCode == 201) {
        ...
        if (responseData is Map<String, dynamic>) {
          productId = responseData['id_product'] as int? ?? 
        ...
        if (images != null && images.isNotEmpty) {
          ...
          for (int i = 0; i < images.length; i++) {
            try {
              ...
              final errorMessage = 'Error al subir imagen ${i + 1}: ${e.toString().replaceFirst('Exception: ', '')}';
              print(errorMessage);
          ...
          if (imageErrors.isNotEmpty) {
            throw Exception('Errores al subir imágenes:\n${imageErrors.join('\n')}');
          }
        }
        ...
        throw Exception('Error al crear producto: ${createResponse.statusCode}');
      }
    } catch (e) {
      _error = e.toString().replaceFirst('Exception: ', '');
      rethrow;
