...
class WalletComponent extends ChangeNotifier {
  ...
  Future<void> fetchWalletData() async {
    ...
    await Future.wait([
      _fetchBalance(),
      _fetchTrm(),
      _fetchWeeklyProductions(),
      _fetchMonthlyProductions(),
      _fetchLiquidations(),
    ]);

    // Segunda ronda: producciones del último período liquidado (depende de _liquidations)
    await _fetchLastLiquidationProductions();
  ...
  Future<void> _fetchWeeklyProductions() async {
  ...
  Future<void> _fetchMonthlyProductions() async {
    ...
    try {
      ...
      if (res.statusCode == 200) {
        ...
        if (list.isNotEmpty && list.first is Map) {
          debugPrint('[Wallet][Production] keys=${(list.first as Map).keys.toList()}');
  ...
  Future<void> _fetchLastLiquidationProductions() async {
    final last = lastLiquidation;
  ...
  Future<void> fetchWeekData(String startDate, String endDate) async {
    _isLoadingWeekData = true;
    notifyListeners();

    _selectedWeekProductions = await _fetchProductions(startDate, endDate);
