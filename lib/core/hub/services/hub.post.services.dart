...
class HubPostsService {
  ...
  Future<HubPostsResponse> fetchPosts({int page = 1, int perPage = 15, String? tag}) async {
    final response = await http.get(_uri(page: page, perPage: perPage, tag: tag), headers: _headers());
    if (response.statusCode >= 400) {
      throw Exception('No fue posible cargar publicaciones (${response.statusCode})');
    }
  ...
  Future<Map<String, dynamic>> postRaw(Uri url, Map<String, dynamic> body) async {
  ...
  Future<void> createPost(Map<String, dynamic> body) async {
    final url = Uri.parse('${EnvironmentDev.baseUrl}${EnvironmentDev.hubPostsList}');
    final response = await http.post(
      url,
      headers: {'Content-Type': 'application/json', ..._headers()},
      body: jsonEncode(body),
    ...
    if (response.statusCode >= 400) {
      throw Exception('No fue posible crear la publicación (${response.statusCode}): ${response.body}');
    }
  }
}
...
class HubPostsResponse {
  ...
  const HubPostsResponse({required this.posts, this.total, this.currentPage, this.lastPage});
}
